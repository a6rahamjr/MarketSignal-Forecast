...
Market analysis prototypes often lack a stable prediction target, repeatable feature set, temporal
evaluation, and a safe model-serving boundary. This makes their output difficult to benchmark,
reproduce, and integrate into other systems.

MarketSignal provides a measurable forecasting workflow. Each run is reproducible, holdout results
are inspectable, and the exact trained behavior is available from both the command line and HTTP.
